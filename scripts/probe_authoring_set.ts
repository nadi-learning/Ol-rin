...
async function main() {
  ...
  const [tut] = await db.insert(appUser).values({ email: `aset-tut-${tag}@example.com`, name: "Tutor" }).returning();
  const [stu] = await db.insert(appUser).values({ email: `aset-stu-${tag}@example.com`, name: "Student" }).returning();
