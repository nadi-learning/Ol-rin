...
const state = async (label) => {
  const s = await p.evaluate(() => ({
    prompt: document.querySelector(".onb-stage-prompt")?.textContent?.trim().slice(0,70),
    buttons: [...document.querySelectorAll("button")].map(e=>`${e.className.split(" ")[0]}|${e.textContent.trim().slice(0,12)}`).slice(0,12),
    field: !!document.querySelector(".onb-field"),
  }));
  console.log(`[${label}] prompt="${s.prompt}" field=${s.field}\n   btns=${JSON.stringify(s.buttons)}`);
};
...
await p.evaluate((e)=>{const el=document.querySelector(".or-dev-input");
  ...
  el.dispatchEvent(new Event("input",{bubbles:true}));}, `dbg4-${Date.now()}@example.com`);
...
await new Promise(r=>setTimeout(r,800));
await state("before That's me");
await p.evaluate(()=>{const c=document.querySelector(".onb-duo-cta"); if(c&&!c.disabled)c.click();});
await new Promise(r=>setTimeout(r,4000));
await state("after That's me (fav_character?)");
await p.evaluate(()=>{const c=document.querySelector(".onb-choice"); if(c)c.click();});
await new Promise(r=>setTimeout(r,4000));
await state("after hero pick");
await p.evaluate(()=>{const c=document.querySelector(".onb-choice"); if(c)c.click();});
await new Promise(r=>setTimeout(r,4500));
await state("after pet pick (phone?)");
await p.screenshot({path:"scratchpad/batch1-shots/dbg-flow.png"});
await b.close();
